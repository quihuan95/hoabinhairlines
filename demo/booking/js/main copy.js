...
$(".item-diem-di").on('click', function() {
  ...
  $(".bt-diem-di").html('<span class="txt-ellipse" data-code="'+html_nn_code+'">' + html_nn + '</span>');
...
$(".item-diem-den").on('click', function() {
  var html_nn = $(this).html();
  var html_nn_code = $(this).data("code");
  $(".bt-diem-den").html('<span class="txt-ellipse" data-code="'+html_nn_code+'">' + html_nn + '</span>');
  $(this).parent().parent().parent().parent().parent().parent().removeClass('open');
  $('body').removeAttr('style');
...
$(".btn-tim-ve").click(function(e){
	...
	var flight_type='';
	var date_go=$("#date_di").val();
	date_go=date_go.replace('/', '-');
	var date_return=$("#date_ve").val();
	date_return=date_return.replace('/', '-');
	var dtpDepartDate= date_go=date_go.replace('/', '-');
	var dtpReturnDate= date_return.replace('/', '-');
	if($("input[name='loai-ve']:checked").val()=='return'){
		flight_type='roundway';
	}else {
		flight_type='oneway';
		dtpReturnDate='';
	}
	var ddlDepartCity = $(".bt-diem-di span").data("code");
	var ddlReturnCity = $(".bt-diem-den span").data("code");
	
	/*var ddlADT = $("#num-adault-xs").val();
	if(ddlADT == undefined){
		ddlADT = $("#num-adault").val();
	}
	
	var ddlCHD = $("#num-child-xs").val();
	if(ddlCHD == undefined){
		ddlCHD = $("#num-child").val();
	}
	
	var ddlINF = $("#num-baby-xs").val();
	if(ddlINF == undefined){
		ddlINF = $("#num-baby").val();
	}*/
	
	var ddlADT = $("#ddlADT").val();
	var ddlCHD = $("#ddlCHD").val();
	var ddlINF = $("#ddlINF").val();	
	//alert(dtpDepartDate);
	var params="flight_type="+flight_type+"&ddlDepartCity="+ddlDepartCity+"&ddlReturnCity="+ddlReturnCity+"&dtpDepartDate="+dtpDepartDate+"&dtpReturnDate="+dtpReturnDate+"&ddlADT="+ddlADT+"&ddlCHD="+ddlCHD+"&ddlINF="+ddlINF;

	
	location.href = "https://hoabinhairlines.vn/flight?"+params;
...
/*$(".btn-tim-ve").click(function(e){
	...
	if(partner == 'maybaytech') {
	  $.post("demo/CheckFlight/checkbooking_new.php", {
		Adt: adault,
		Chd: child,
		Departure: diem_di,
	  ...
	  if(itineraryType == 2){
		  window.location.href = "demo/CheckFlight/checkbooking_new.php?"+diem_di+diem_den+date_di+"-"+diem_den+diem_di+date_ve+"-"+adault+"-"+child+"-"+baby;
		  if(search_month) {
			window.location.href = "demo/CheckFlight/checkbooking_new.php?"+diem_di+diem_den+search_month_go+"-"+diem_den+diem_di+search_month_back+"-"+adault+"-"+child+"-"+baby;
		  }
	  } else {
		  window.location.href = "demo/CheckFlight/checkbooking_new.php?"+diem_di+diem_den+date_di+"-"+adault+"-"+child+"-"+baby;
		  if(search_month) {
			window.location.href = "demo/CheckFlight/checkbooking_new.php?"+diem_di+diem_den+search_month_go+"-"+adault+"-"+child+"-"+baby;
		  }
	  }
	}
})*/
...
$("#date_ve, #date_ve_taxi, #date_ve_road").click(function(){
  if ($(window).width() < 768) {
	  ...
	  $("#main-nav-date").show().find("span").html("Chá»n ngĂ y Ä‘i");
	  $(".overlay-body").show();
...
$("#date_di, #start_date, #date_di_taxi, #date_di_road").click(function(){
  if ($(window).width() < 768) {
	  $("#main-nav").hide();
	  $(".header").removeClass("nav-up");
	  $("#main-nav-date").show().find("span").html("Chá»n ngĂ y vá»");
	  $(".overlay-body").show();
	  $("body, html").addClass("overflow_hidden");
	  document.body.addEventListener("touchmove", freezeVp, false);
	  $("body,html").animate({
...
function onDateTimePicker() {
	...
	date_input_ve.change(function(){
		...
		if(_datetime_calendar_back) {
		  ...
		  if(_date_di_ > _date_ve_) {
			alert("NgĂ y vá» pháº£i báº±ng hoáº·c sau ngĂ y Ä‘i");
