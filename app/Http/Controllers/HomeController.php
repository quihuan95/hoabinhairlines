...
class HomeController extends Controller
{
    ...
    public function index(){
        ...
        if($canonical=='https://hoabinhairlines.vn/index.php'){
    ...
    public function flight_data(Request $request){
        $name = $request->input('ddlReturnCity');
        $data=DB::table('countries')->where("pid","!=",0)->where("title","LIKE","%".$name."%")->get();
        $jdata=array();
        if(count($data)>0){
            foreach($data as $k=>$v){
              $jdata[]=$v->title;
    ...
    public function pages($slug){
        ...
        if(($slug!="gioi-thieu") && ($slug!="thong-tin-chuyen-khoan") && ($slug!="huong-dan-dat-ve")){
            if(!empty($first_pages)){
                ...
                $canonical=url()->current();
                $og_image='https://hocvienbolerovietnam.com/public/uploads/banner/khoa-2.jpg';
